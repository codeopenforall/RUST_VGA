...
fn main() {
    let packet = Arc::new(DataPacket::new("Hello", "olleH", "reverse"));
    let shared_packet = Arc::clone(&packet);
    let handler = thread::spawn(move || {
        if shared_packet.validate() {
            println!("Validation succeeded: signature correctly accepted.");
        } else {
            println!("Validation failed (unexpected for a valid signature).");
        }
    });
