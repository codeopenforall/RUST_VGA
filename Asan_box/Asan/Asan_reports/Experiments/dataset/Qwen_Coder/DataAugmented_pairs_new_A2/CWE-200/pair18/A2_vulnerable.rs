...
fn process_data(data: Arc<Data>) {
    unsafe {
        ...
        if secret_copy.contains("vulnerable") {
            // Introducing a CWE-787: Improper Neutralization of Special Elements used in an OS Command ('OS Command Injection')
            // by including the secret in a command string that could be executed.
            let cmd = format!("echo {}", secret_copy);
