...
fn constant_time_compare(stored: &str, provided: &str) -> bool {
    let n = stored.len().max(provided.len());
    let mut result: u8 = 0;
    for i in 0..n {
        let sc = if i < stored.len() { stored.as_bytes()[i] } else { 0 };
        let pc = if i < provided.len() { provided.as_bytes()[i] } else { 0 };
        result |= sc ^ pc;
    }
...
impl AccessControl {
    ...
    pub fn authenticate(&self, username: &str, password: &str) -> Result<(), &'static str> {
        ...
        if !stored.is_empty() && constant_time_compare(stored, password) {
            Ok(())
        } else {
