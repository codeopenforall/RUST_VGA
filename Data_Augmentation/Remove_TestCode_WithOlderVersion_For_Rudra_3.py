...
"""
Run_Oracles.py
Compiles and runs test oracles for generated Rust pairs using a pinned
Rust toolchain (nightly-2021-10-21), so it is compatible with Rudra.
...
We verify the contract per file:
  * fixed.rs       SHOULD PASS the tests  → if not, delete ONLY fixed.rs
  * vulnerable.rs  SHOULD FAIL the tests  → if not, delete ONLY vulnerable.rs
...
Notes:
- We NEVER delete directories or demo_test.rs; only offending *.rs files.
...
DEFAULT_ROOT = "/home/ikqnm/PycharmProjects/rudra_sandbox/rudra_big/rudra_big/src/dataset/Qwen_Coder/DataAugmented_pairs_new_A3/CWE-908"
RUSTC_EDITION = "2021"
TIMEOUT_SECS = 600  # seconds per compile/run

# Toolchain pinned to Rudra-compatible nightly
RUST_TOOLCHAIN = "nightly-2021-10-21"
...
def compile_and_run_single(rs_file: Path, oracle_file: Path) -> Dict[str, Any]:
    """
    Concatenate rs_file + oracle_file into a temporary source, then compile with
    `rustup run nightly-2021-10-21 rustc --test` and run the produced test binary.
    ...
    with tempfile.TemporaryDirectory(prefix="oracle_") as td:
        ...
        # Use the pinned toolchain via rustup
        cmd = [
            "rustup",
            "run",
            RUST_TOOLCHAIN,
...
def process_root(root: Path, delete_bad: bool = True, dry_run: bool = False):
    ...
    for dirpath, _dirnames, filenames in os.walk(root):
        ...
        print(
            f"[fixed] compiled={f_res['compiled']} ran={f_res['ran']} "
            f"ok={f_res['ok']} phase={f_res['phase']}"
        )
        if f_res.get("output"):
            print("---- fixed logs ----")
            print(f_res["output"])
        print(
            f"[vuln ] compiled={v_res['compiled']} ran={v_res['ran']} "
            f"ok={v_res['ok']} phase={v_res['phase']}"
        )
        if v_res.get("output"):
            print("---- vuln logs ----")
            print(v_res["output"])

        fixed_ok, vuln_fail, fixed_reason, vuln_reason = contract_decisions(
            f_res, v_res
        )
        ...
        if not del_fixed and not del_vuln:
            print(
                "[decision] KEEP (fixed PASSED, vulnerable FAILED as expected)"
            )
        else:
            print("[decision] BAD (contract not met)")
            if del_fixed:
                print(
                    f"  -> {'would delete' if dry_run or not delete_bad else 'delete'}: "
                    f"{fixed_path.name}  (reason: {fixed_reason})"
                )
                will_delete_files.append((fixed_path, fixed_reason))
            if del_vuln:
                print(
                    f"  -> {'would delete' if dry_run or not delete_bad else 'delete'}: "
                    f"{vuln_path.name}   (reason: {vuln_reason})"
                )
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Compile and run tests for generated pairs (delete only offending files)."
    )
    ap.add_argument(
        "--root",
        default=DEFAULT_ROOT,
        help="Root folder containing generated outputs",
    )
    ap.add_argument(
        "--delete-bad",
        action="store_true",
    ...
    ap.add_argument(
        "--dry-run",
        action="store_true",
        help="Print what would be deleted, but do not delete",
    )
    args = ap.parse_args()
    ...
    # We depend on rustup rather than bare rustc, because we pin a toolchain.
    check_cmd_exists("rustup")

    # Show which compiler version we are actually using.
    try:
        ...
        print(
            f"ERROR: failed to query rustc version for toolchain {RUST_TOOLCHAIN}: {e}",
            file=sys.stderr,
        )
        sys.exit(1)

    print(f"[info] Using toolchain {RUST_TOOLCHAIN}: {version}")
