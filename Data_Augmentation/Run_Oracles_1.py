...
"""
Run_Oracles.py
Compiles and runs test oracles for generated A1 Rust pairs.
...
We verify the contract per file:
  * A1_fixed.rs       SHOULD PASS the tests  → if not, delete ONLY A1_fixed.rs
  * A1_vulnerable.rs  SHOULD FAIL the tests  → if not, delete ONLY A1_vulnerable.rs
...
Notes:
- We NEVER delete directories or demo_test.rs; only offending A1_*.rs files.
...
DEFAULT_ROOT = "/home/ikqnm/PycharmProjects/rudra_sandbox/rudra_big/rudra_big/src/dataset/o3_mini/data_generation/CWE-134/"
RUSTC_EDITION = "2021"
...
def compile_and_run_single(rs_file: Path, oracle_file: Path) -> Dict[str, Any]:
    ...
    """
    with tempfile.TemporaryDirectory(prefix="a1_oracle_") as td:
        td_path = Path(td)
...
def process_root(root: Path, delete_bad: bool = True, dry_run: bool = False):
    ...
    for dirpath, _dirnames, filenames in os.walk(root):
        files = set(filenames)
        if not {"A1_fixed.rs", "A1_vulnerable.rs"}.issubset(files):
            continue
        ...
        fixed_path = p / "A1_fixed.rs"
        vuln_path  = p / "A1_vulnerable.rs"
        ...
        if not del_fixed and not del_vuln:
            print("[decision] KEEP (A1_fixed PASSED, A1_vulnerable FAILED as expected)")
        else:
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description="Compile and run tests for generated A1 pairs (delete only offending files).")
